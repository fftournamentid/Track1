...
Workflows are managed by Replit and started automatically. To run manually:

- `pnpm --filter @workspace/api-server run dev` — run the API server (Replit assigns `PORT` automatically; defaults to 8080 in dev)
- `pnpm run typecheck` — full typecheck across all packages (always run from root; per-package typecheck requires lib references to be built first)
- `pnpm run build` — typecheck + build all packages
- `pnpm --filter @workspace/api-spec run codegen` — regenerate API hooks and Zod schemas from the OpenAPI spec
- `pnpm --filter @workspace/db run push` — push DB schema changes (dev only)
- Required env: `DATABASE_URL` — Postgres connection string (provision via Replit database integration before using `lib/db` or the API server)
